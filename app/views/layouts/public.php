...
<head>
    ...
    <title>APP-ROOMS · Alojamiento universitario verificado</title>
...
<body class="pd-body">
<?php
/* ── Session-aware data for header ── */
$_pd_logueado = !empty($_SESSION['usuario_id']);
$_pd_uri      = $_SERVER['REQUEST_URI'] ?? '/';
...
if ($_pd_logueado) {
    ...
    try {
        $_pd_noLeidos = (int)(new \App\Models\Chat())->contarNoLeidos($_SESSION['usuario_id']);
    } catch (\Throwable $_pd_e) { $_pd_noLeidos = 0; }

    // Avatar
    $_pd_foto = $_SESSION['url_foto'] ?? null;
    $_pd_nom = $_SESSION['nombres'] ?? '';
    $_pd_ape = $_SESSION['apellidos'] ?? '';
    $_pd_mb  = function_exists('mb_substr');
    $_pd_avatar_letras = strtoupper(
        ($_pd_mb ? mb_substr($_pd_nom, 0, 1) : substr($_pd_nom, 0, 1)) .
        ($_pd_mb ? mb_substr($_pd_ape, 0, 1) : substr($_pd_ape, 0, 1))
    );
    $_pd_nombre_completo = $_pd_nom . ' ' . $_pd_ape;
}
?>
    ...
    <header class="pd-header" id="pdHeader">
        <div class="pd-header-inner">
            ...
            <?php if ($_pd_logueado): ?>
            <!-- ═══ Logged-in nav ═══ -->
            <div class="pd-header-right" style="display:flex;align-items:center;gap:6px">
                <nav class="pd-nav">
                    <a href="/" class="pd-nav-link <?php echo ($_pd_uri === '/') ? 'is-active' : ''; ?>">Inicio</a>
                    <a href="/dashboard" class="pd-nav-link <?php echo (strpos($_pd_uri, '/dashboard') !== false) ? 'is-active' : ''; ?>">Dashboard</a>
                    <a href="/buscar" class="pd-nav-link <?php echo (strpos($_pd_uri, '/buscar') !== false) ? 'is-active' : ''; ?>">Buscar</a>
                    <a href="/foros" class="pd-nav-link <?php echo (strpos($_pd_uri, '/foros') !== false) ? 'is-active' : ''; ?>">Comunidad</a>
                    <a href="/mensajes" class="pd-nav-link <?php echo (strpos($_pd_uri, '/mensajes') !== false) ? 'is-active' : ''; ?>" style="position:relative">Mensajes<?php if ($_pd_noLeidos > 0): ?><span style="position:absolute;top:-6px;right:-12px;background:var(--pd-accent);color:#fff;font-size:10px;font-weight:700;min-width:17px;height:17px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;padding:0 4px"><?php echo $_pd_noLeidos > 9 ? '9+' : $_pd_noLeidos; ?></span><?php endif; ?></a>
                ...
                <div class="dropdown">
                    <div class="pd-avatar-btn" data-bs-toggle="dropdown" aria-expanded="false" style="cursor:pointer">
                        <?php if (!empty($_pd_foto)): ?>
                            <img src="<?php echo htmlspecialchars($_pd_foto); ?>" alt="avatar">
                        <?php else: ?>
                            <span><?php echo htmlspecialchars($_pd_avatar_letras); ?></span>
                        <?php endif; ?>
                    </div>
                    <ul class="dropdown-menu dropdown-menu-end" style="border-radius:var(--pd-r-md);border:1px solid var(--pd-line);box-shadow:var(--pd-sh-2);min-width:200px;padding:8px 0">
                        <li style="padding:8px 16px;font-weight:600;font-size:14px;color:var(--pd-ink)"><?php echo htmlspecialchars($_pd_nombre_completo); ?></li>
                        ...
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="/logout" style="color:var(--pd-accent)"><i class="fas fa-sign-out-alt fa-sm fa-fw me-2"></i> Cerrar sesión</a></li>
                    </ul>
                </div>
            </div>
            ...
            <!-- ═══ Guest nav ═══ -->
            <nav class="pd-nav">
                <a href="/" class="pd-nav-link">Inicio</a>
                <a href="/buscar" class="pd-nav-link">Buscar</a>
                <a href="/foros" class="pd-nav-link">Comunidad</a>
                <a href="/login" class="pd-nav-link">Iniciar sesión</a>
                <a href="/register" class="pd-btn pd-btn-primary" style="padding:10px 18px;font-size:14px">Soy estudiante</a>
            </nav>
            <?php endif; ?>
        </div>
    </header>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-2 col-md-3 col-6">
                    ...
                    <ul>
                        ...
                        <li><a href="#">Soy propietario</a></li>
                    </ul>
                </div>
                <?php if ($_pd_logueado): ?>
                <div class="col-lg-2 col-md-3 col-6">
    ...
    <script>
        ...
        // Confirmación destructiva con SweetAlert2 (from main.php)
        function confirmarAccionSweet(event, titulo, texto, icono, textConfirm, btnColor) {
            icono = icono || 'warning';
            textConfirm = textConfirm || '<i class="fas fa-check me-1"></i> Sí, continuar';
            btnColor = btnColor || '#EF4444';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({ title: 'Procesando...', text: 'Por favor espera un momento', allowOutsideClick: false, allowEscapeKey: false, showConfirmButton: false, didOpen: function () { Swal.showLoading(); } });
                    form.submit();
                }
            });
            return false;
